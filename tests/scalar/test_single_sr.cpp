...
void test_op(const std::vector<T> &args, int op, int num_args) {
  ...
  switch (op) {
  case 0:
    fprintf(stderr, fmt, prism::sr::scalar::add(args[0], args[1]));
    break;
  case 1:
    fprintf(stderr, fmt, prism::sr::scalar::sub(args[0], args[1]));
    break;
  case 2:
    fprintf(stderr, fmt, prism::sr::scalar::mul(args[0], args[1]));
    break;
  case 3:
    fprintf(stderr, fmt, prism::sr::scalar::div(args[0], args[1]));
    break;
  case 4:
    fprintf(stderr, fmt, prism::sr::scalar::sqrt(args[0]));
    break;
  case 5:
    fprintf(stderr, fmt, prism::sr::scalar::fma(args[0], args[1], args[2]));
    break;
  }
